...
import com.aoserver.utils.ByteMigration;

import java.io.UnsupportedEncodingException;
import java.nio.ByteBuffer;
import java.nio.charset.StandardCharsets;
import static com.aoserver.utils.ByteMigration.*;

/**
 * Aca es donde se gestiona toda la cola de bytes que entran y salen.
 */
public class ByteQueue {
    ...
    public int writeInteger(short value) {
        byte[] buf = new byte[2];
        ByteBuffer.wrap(buf).putShort(ByteMigration.bigToLittle_Short(value));
        return writeData(buf, 2);
    }

    public int writeLong(int value) {
        byte[] buf = new byte[4];
        ByteBuffer.wrap(buf).putInt(ByteMigration.bigToLittle_Int(value));
        return writeData(buf, 4);
    }

    public int writeSingle(float value) {
        byte[] buf = new byte[4];
        ByteBuffer.wrap(buf).putFloat(ByteMigration.bigToLittle_Float(value));
        return writeData(buf, 4);
    }

    public long writeDouble(double value) {
        byte[] buf = new byte[8];
        ByteBuffer.wrap(buf).putDouble(ByteMigration.bigToLittle_Double(value));
    ...
    public int readByte() {
        ...
        return ByteMigration.bigToLittle_Byte(ByteBuffer.wrap(buf).get()) & 0xFF;
    }

    public short readInteger() {
        byte[] buf = new byte[2];
        removeData(readData(buf, 2));
        return ByteMigration.bigToLittle_Short(ByteBuffer.wrap(buf).getShort());
    }

    public int readLong() {
        byte[] buf = new byte[4];
        removeData(readData(buf, 4));
        return ByteMigration.bigToLittle_Int(ByteBuffer.wrap(buf).getInt());
    }

    public float readFloat() {
        byte[] buf = new byte[4];
        removeData(readData(buf, 4));
        return ByteMigration.bigToLittle_Float(ByteBuffer.wrap(buf).getFloat());
    }
    ...
    public String readASCIIString() {
        ...
        if (queueLength > 1) {
            readData(buf, 2);
            length = ByteMigration.bigToLittle_Short(ByteBuffer.wrap(buf).getShort());
